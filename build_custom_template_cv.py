...
def generate_exact_custom_cv():
    ...
    # EXACT COLOR PALETTE extracted from CV_TELLENG ALBA.docx
    SIDEBAR_FILL = "2F3B4A"      # Slate Charcoal Dark
    MAIN_FILL = "F5EDE8"         # Warm Cream Off-White
    
    GOLD_HEADER = RGBColor(0xB8, 0x94, 0x5A)       # Warm Gold for Sidebar Headers
    LIGHT_TEXT = RGBColor(0xE8, 0xDD, 0xD8)        # Light Warm Gray for Sidebar Text
    GOLD_DECO = RGBColor(0xD4, 0xB8, 0x8C)         # Soft Gold for Bullets/Lines
    
    CRIMSON_MAIN = RGBColor(0xC1, 0x7C, 0x74)      # Crimson Rose for Main Section Headings & Badges
    ROSE_DECO = RGBColor(0xD9, 0xA2, 0x9D)         # Rose Gold for Line Dividers
    DARK_TITLE = RGBColor(0x33, 0x33, 0x33)        # Dark Charcoal for Main Titles
    BODY_DARK = RGBColor(0x4A, 0x4A, 0x4A)         # Dark Gray for Body Paragraphs
    SUBTLE_TEXT = RGBColor(0x77, 0x77, 0x77)       # Muted Gray

    table = doc.add_table(rows=2, cols=2)
    table.alignment = WD_TABLE_ALIGNMENT.CENTER
    remove_table_borders(table)

    col_widths = [Inches(2.6), Inches(4.8)]
    ...
    add_sb_t(c00, "Français   ■ ■ ■ ■ ■")
    add_sb_t(c00, "Anglais    ■ ■ ■ ■ □")
    ...
    r_nm.font.color.rgb = CRIMSON_MAIN
    ...
    r_sb.font.bold = True
    r_sb.font.color.rgb = SUBTLE_TEXT
    ...
    def add_mn_h(cell, title):
        ...
        r1.font.bold = True
        r1.font.size = Pt(9.5)
        r1.font.color.rgb = CRIMSON_MAIN
        r2 = p.add_run(title.upper())
        r2.font.bold = True
        r2.font.size = Pt(10)
        r2.font.color.rgb = DARK_TITLE
    ...
    def add_proj(cell, name, sub_badge, bullets):
        ...
        r1.font.bold = True
        r1.font.size = Pt(9.5)
        r1.font.color.rgb = DARK_TITLE
        
        r2 = p.add_run("–  " + sub_badge)
        r2.font.italic = True
        r2.font.size = Pt(8.5)
        r2.font.color.rgb = CRIMSON_MAIN
        ...
        for b in bullets:
            ...
            rb_ico.font.color.rgb = CRIMSON_MAIN
            rb_txt = bp.add_run(b)
    ...
    r_sb_nm.font.color.rgb = GOLD_HEADER
    ...
    r_sbd.font.color.rgb = GOLD_DECO
    ...
    # Output paths
    f1 = r"c:\Users\HP\Desktop\portfolio-gervais\KOA_MARIE_GERVAIS_NELLY_CV_DESIGN_EXACT.docx"
    doc.save(f1)
    print(f"Generated exact design CV at: {f1}")

if __name__ == "__main__":
    generate_exact_custom_cv()
